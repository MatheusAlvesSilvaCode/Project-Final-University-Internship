...
# Função para mostrar conteúdo da estação filtrada
def mostrar_conteudo_estacao_filtrada(estacao_selecionada, evento, apenas_tabelas=False):
    ...
    return html.Div([
        dbc.Row([
            dbc.Col([
                dcc.Graph(figure=figura_aceleracao)
            ], width=6),
            dbc.Col([
                dcc.Graph(figure=figura_frequencia)
            ], width=6)
        ]),
        dbc.Row([
            dbc.Col([
                html.H5("Valores de Pico de Aceleração [mg]", style={"textAlign": "center"}),
                dbc.Table([
                    html.Thead(html.Tr([html.Th("Direção"), html.Th("Valor")])),
                    html.Tbody([
                        html.Tr([
                            html.Td(direcao),
                            html.Td(f"{tabela_picos.loc[direcao, 'peak']:.3f}" if pd.notna(tabela_picos.loc[direcao, 'peak']) else '-')
                        ]) for direcao in ['T','R','V']
                    ])
                ], bordered=True, style={"marginBottom": "20px"})
            ], width=6),
            dbc.Col([
                ...
                dbc.Table([
                    ...
                    html.Tbody([
                        html.Tr([
                            html.Td(direcao),
                            html.Td(f"{tabela_fatores.loc[direcao, 'valor']:.3f}" if pd.notna(tabela_fatores.loc[direcao, 'valor']) else '-')
                        ]) for direcao in ['T','R','V']
                    ])
                ], bordered=True, style={"marginBottom": "20px"})
            ], width=6)
...
def selecionar_todas_freq(selecionar_todas, opcoes):
    ...
    html_parts.append(f"<h2>RELATÓRIO DE EVENTO SÍSMICO DA BARRAGEM DE DAIVÕES</h2>")
        ...
        if not df_freq_consolidated.empty:
            ...
            if not freq_filtradas.empty:
                ...
                for direcao in ["T", "R", "V"]:
                    ...
                    fig.update_layout(
                        title=f"Espectro de Frequência - FFT Direção {direcao} (Evento: {data_hora_formatada}, Estação: {estacao_nome})",
                        title_xaxis="Frequência (Hz)",
                        title_yaxis="Aceleração (mg)",
                        margin=dict(l=40, r=40, t=40, b=40),
                        height=300,
                        plot_bgcolor='white',
                        yaxis=dict(range=[y_min_freq, y_max_freq], showgrid=True, gridcolor='#e5e5e5', gridwidth=0.7),
