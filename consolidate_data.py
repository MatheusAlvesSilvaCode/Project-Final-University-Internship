...
for root, dirs, files in os.walk(base_path):
    for file in files:
        # Verifica se o arquivo termina com '_data.csv'
        if file.endswith("_data.csv"):
            ...
            try:
                ...
                estacao = partes[1].replace("_data.csv", "")  # Ex: 20160003
...
if dfs:
    df_geral_data = pd.concat(dfs, ignore_index=True)
    #print(f"Total de linhas consolidadas: {len(df_geral_data)}")

    # Salva o DataFrame consolidado em um novo arquivo CSV
    df_geral_data.to_csv("data_consolidado.csv", index=False)
else:
    print("Nenhum arquivo '_data.csv' encontrado.")  # Mensagem se não encontrar arquivos
